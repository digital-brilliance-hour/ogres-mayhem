...
int check_roped(void player) {
    int anim_id = getentityproperty(player, "animationid");

            if ( anim_id == openborconstant("ANI_PAIN62") || anim_id == openborconstant("ANI_PAIN63") ) {
                ...
                if ( getentityvar(player,1) == NULL() ) { // se non abbiamo impostato il tempo...
                    //---------PRESS BUTTON-----------
                    if ( getglobalvar("roped_button_escape"+p) == NULL() && getglobalvar("roped_active"+p) == NULL() ) {
                        void ent = spawnbuttons("press_any_button", player);
                        setglobalvar("roped_button_escape"+p, ent);
                    }
                    //--------------------------------
                        ...
                        if ( playerkeys(p,1,"attack") == 64 || playerkeys(p,1,"moveleft") == 4 || playerkeys(p,1,"moveright") == 8 || playerkeys(p,1,"moveup") == 16
                          || playerkeys(p,1,"movedown") == 32 || playerkeys(p,1,"jump") == 128 || playerkeys(p,1,"special") == 256 ) {
                            ...
                            if ( getentityproperty(player,"health") > 0 ) {
                                if ( anim_id != openborconstant("ANI_PAIN63") ) performattack(player,openborconstant("ANI_PAIN63"),1);
                            }
                        } // fine if playerkeys
                    } // fine if pain32

                    if ( getentityvar(player, 2) > combo_max ) { // LIBERIAMOCI grazie alla combo!!
                        if ( getglobalvar("roped_button_escape"+p) != NULL() && getentityproperty(getglobalvar("roped_button_escape"+p), "exists") ) killentity(getglobalvar("roped_button_escape"+p)); // Eliminiamo l'animazione pulsante
                        setglobalvar("roped_button_escape"+p, NULL());
                        setglobalvar("roped_active"+p, 1);
                    ...
                    setentityvar(player,2,NULL()); // Annulliamo la combo
                    // Cambiamo animazione
                    if ( anim_id != openborconstant("ANI_PAIN62") ) changeentityproperty(player,"animation",openborconstant("ANI_PAIN62"));
                } // fine if tempo
            } else if ( anim_id != openborconstant("ANI_PAIN62") && anim_id != openborconstant("ANI_PAIN63") ) { // Eliminiamo l'animazione pulsante per prevenire bugs
                int p = getentityproperty(player, "playerindex");
...
int spawn_destroyed_obj(void self) {
    void subent = spawnsubentity_relative("rope_explo",0,1,0,self);
}
