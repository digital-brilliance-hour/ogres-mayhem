...
int check_frozen(void player) {
    int anim_id = getentityproperty(player, "animationid");

            if ( anim_id == openborconstant("ANI_PAIN34") || anim_id == openborconstant("ANI_PAIN33") ) {
                ...
                if ( getentityvar(player,1) == NULL() ) { // se non abbiamo impostato il tempo...
                    //---------PRESS BUTTON-----------
                    if ( getglobalvar("frozen_button_escape"+p) == NULL() && getglobalvar("frozen_active"+p) == NULL() ) {
                        void ent = spawnbuttons("press_any_button", player);
                        setglobalvar("frozen_button_escape"+p, ent);
                    }
                    //--------------------------------
                    setentityvar(player,1,get_next_time(reset_time));
                } else if ( getentityvar(player,1) > time ) {
                    if ( anim_id == openborconstant("ANI_PAIN34") ) {
                        // Premiamo i pulsanti
                        if ( playerkeys(p,1,"attack") == 64 || playerkeys(p,1,"moveleft") == 4 || playerkeys(p,1,"moveright") == 8 || playerkeys(p,1,"moveup") == 16
                          || playerkeys(p,1,"movedown") == 32 || playerkeys(p,1,"jump") == 128 || playerkeys(p,1,"special") == 256 ) {
                            ...
                            if ( getentityproperty(player,"health") > 0 ) performattack(player,openborconstant("ANI_PAIN33"),1);
                        } // fine if playerkeys
                    } // fine if pain32

                    if ( getentityvar(player, 2) > combo_max ) { // LIBERIAMOCI grazie alla combo!!
                        if ( getglobalvar("frozen_button_escape"+p) != NULL() && getentityproperty(getglobalvar("frozen_button_escape"+p), "exists") ) killentity(getglobalvar("frozen_button_escape"+p)); // Eliminiamo l'animazione pulsante
                        setglobalvar("frozen_button_escape"+p, NULL());
                        setglobalvar("frozen_active"+p, 1);
                    ...
                    setentityvar(player,2,NULL()); // Annulliamo la combo
                    // Cambiamo animazione
                    if ( anim_id != openborconstant("ANI_PAIN34") ) changeentityproperty(player,"animation",openborconstant("ANI_PAIN34"));
                } // fine if tempo
            } else if ( anim_id != openborconstant("ANI_PAIN33") && anim_id != openborconstant("ANI_PAIN34") ) { // Eliminiamo l'animazione pulsante per prevenire bugs
                int p = getentityproperty(player, "playerindex");
...
int spawn_destroyed_obj(void self) {
    void subent = spawnsubentity_relative("frozen_explo",0,1,0,self);
}
